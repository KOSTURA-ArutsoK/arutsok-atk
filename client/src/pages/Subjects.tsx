...
export default function Subjects() {
  ...
  useEffect(() => {
    ...
    if (pendingAddNew.clientType && pendingAddNew.baseValue) {
      ...
      let aresData: any;
      try {
        const raw = sessionStorage.getItem('pridat_subjekt_ares');
        if (raw) { aresData = JSON.parse(raw); sessionStorage.removeItem('pridat_subjekt_ares'); }
      } catch {}
      setEditData({ clientTypeCode: pendingAddNew.clientType, stateId: appUser.activeStateId, baseValue: pendingAddNew.baseValue, aresData });
    } else {
      setAddNewHandled(true);
