...
- **10-Level Sentinel Security Pyramid**: `SENTINEL_LEVELS` (L0-L9) defined in `shared/schema.ts`. `getSecurityLevel(appUser)` resolves from `securityLevel` column or role fallback. L0=Public, L1=Potential, L2=Tipér, L3=Obchodník, L4=Manažér, L5=Riaditeľ, L6=Backoffice, L7=Architekt, L8=Auditor(ReadOnly), L9=System. Data masking via `maskSensitiveFields()` hides birthNumber/IBAN/account for <L6 non-owners. IP Locking via `allowedIps` column with middleware enforcement. Auditor (L8) blocked from all write operations. Sentinel badge in sidebar footer with color-coded levels.
