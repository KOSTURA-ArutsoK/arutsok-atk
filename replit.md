...
## Recent Changes
- **Client Groups & Reputation System**: System groups (`Klienti`, `Registrovaní klienti`) with `groupCode` and `isSystem=true`, auto-seeded on startup. Contract creation auto-adds subject to "Klienti" group. System groups cannot be deleted. `clientGroups.customFields` (JSONB) for per-group custom field definitions with UI tab. `subjects.redListCompanyId` for local red list per company. `effectiveListStatus` computed field in subjects list/detail responses (filters červený per activeCompanyId). Bonita auto-trigger: bonitaPoints ≤ -5 auto-sets červený zoznam. Blacklist (čierny) blocks contract creation (403). Visual signaling: Ban/AlertTriangle icons in Subjects, Contracts, ContractForm, ContractInventories. ClientGroups page shows Lock icon + "Systémová" badge for system groups, disabled delete.
