...
- **Module-Specific Features**:
    ...
    - **Dynamic Parameter System (ArutsoK 24-25)**: 3-level hierarchy: Sektory → Produkty → Parametre. `sectors` (with `partnerIds` for multi-company assignment), `sector_products` (name, abbreviation, sectorId), `sector_product_parameters` bridge table. Legacy `sector_parameters`, `product_sectors`, `product_parameters` kept for compatibility. Sectors.tsx has 4-tab UI with partner multi-select combobox, sector-filtered product list, parameter CRUD with renamed "Predvolena hodnota k danemu parametru" field + info tooltip, and Panels management. All management tables default to descending sort (newest first).
