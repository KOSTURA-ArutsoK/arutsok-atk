...
- **10-Level Sentinel Security Pyramid**: `SENTINEL_LEVELS` (L0-L9) defined in `shared/schema.ts`. `getSecurityLevel(appUser)` resolves from `securityLevel` column or role fallback. Role mapping: `user→L3`, `admin/superadmin→L5`, `prezident→L6`, `architekt→L7`, `auditor→L8`, `system→L9`. Async `maskSensitiveFields()` with `isSubjectAccessible()` — checks ownership (`uploadedByUserId`, `linkedSubjectId`) AND `managerId` hierarchy chain (up to 10 levels, company-scoped). Non-owners under L6: birthNumber→"******", IBAN→masked, phone→masked, account fields→"****". IP Locking via `allowedIps` with "Sentinel IP Lockout" message. L8 Auditor read-only middleware. Sentinel badge in sidebar footer: amber L7+, yellow L6, blue L3-L5, gray L0-L2. L6 IP warning in Users form.
