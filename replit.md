...
- **Universal Guardian Access Hierarchy**: Both FO (persons) and PO (organizations) can serve as legal guardians. Features: GuardianshipSection component (wards list with search/pagination, guardians list, history dialog), enhanced MaturityAlertsSection with detach/retain resolution dialog (legal basis input, archival), bulk-optimized queries with indexes. `guardianship_archive` table stores complete history with archival triggers (maturity/manual). Guardian access hierarchy grants automatic read-only access to all linked wards. API: `/api/guardianship/wards/:id` (paginated with search), `/api/guardianship/guardians/:id`, `/api/guardianship/history/:id`, POST `/api/guardianship/detach`.
