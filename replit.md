...
- **Module-Specific Features**:
    ...
    - **Dynamic Subject Parameter Library**: Database-driven field definitions replacing hardcoded `staticFieldDefs.ts`. Architecture: `subject_param_sections` (folders/panels) → `subject_parameters` (field definitions) → `subject_templates` (field groupings) → `subject_template_params` (template↔parameter bindings with temporal validity via `validFrom`/`validTo`). Admin UI embedded within Sectors page under "Sektory Subjektov" tab with 4 sub-tabs: Parametre, Sekcie & Panely, Šablóny, Neznáme polia. Supports `getResolvedParametersForTemplate()` for contract-date-based field resolution. Auto-code generation: system generates unique slugs from user-friendly names (`generateAutoCode()` with prefixes `sec_`, `f_`, `p_`, `tmpl_`). Includes `parameter_synonyms` table for AI synonym mapping, `isCollection` flag for multi-value fields, `extractionHints` JSON for regex patterns, and `POST /api/ai/extract-fields` endpoint for document text extraction via synonyms.
    - **Comprehensive Parameter Seed**: `server/seed-subject-params.ts` seeds 304 parameters across FO/SZČO/PO client types covering insurance sectors (SDS, PZP, Život, Majetok, AML), 56 sections, and synonym groups. Triggered via `POST /api/admin/seed-subject-parameters` or UI button.
    - **Parameter Delete Protection**: `DELETE /api/subject-parameters/:id` checks `subject_field_history` for usage before deletion. Returns 400 with count of existing records if parameter is in use.
    - **Unknown Extracted Fields Module**: `unknown_extracted_fields` table tracks fields that AI extraction couldn't match to known parameters. Full CRUD via `/api/unknown-extracted-fields`. AI extraction endpoint auto-saves unmatched lines. Frontend "Neznáme polia" tab with status filtering (new/assigned/dismissed), one-click assign-to-parameter workflow, and dismiss/delete actions.
