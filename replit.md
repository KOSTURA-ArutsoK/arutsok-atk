...
- **IČO Validation + ARES Lookup**: Global `validateSlovakICO()` in `shared/ico-validator.ts` — weighted checksum (weights 8→2), MOD11, auto-padding to 8 digits. ARES lookup endpoint (`GET /api/lookup/ico/:ico`) fetches company name, address, legal form, and DIČ from Czech ARES registry. Used in: import endpoint (icoCritical flag), Subject registration dialog (on-the-fly + ARES banner), SZČO firm section (onBlur + ARES "Použiť údaje" button), backend subject create/update (HTTP 400), and DynamicFieldInput (onBlur).
