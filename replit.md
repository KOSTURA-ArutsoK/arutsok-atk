...
- **Subject UID Lifecycle (KRITICKÉ)**: `subjects.uid` je nullable. UID sa NEPRIDEĽUJE pri vytvorení subjektu cez import. UID sa prideľuje AŽ pri prechode zmluvy do Folderu 2 (fáza 5, `receivedByCentralAt`) alebo Folderu 3 (fáza 6, presun do spracovania). Subjekty bez UID = čakajú na prijatie v centrále. Endpoint `POST /api/admin/merge-duplicate-subjects` zlučuje duplicitné subjekty podľa RČ/IČO (soft-delete duplikátov, reassign zmlúv na kanonický). UI tlačidlo v Settings.tsx (len superadmin).
- **Import UPSERT (Jedno RČ = Jeden subjekt)**: Pri importe (`POST /api/contracts/import-excel`) sa najprv kontroluje `checkDuplicateSubject`. Ak subjekt s daným RČ/IČO existuje → reuse existing `subjectId`. Ak neexistuje a máme dostatok dát (meno+priezvisko+RČ alebo firma+IČO) → vytvorí sa nový subjekt BEZ UID cez `storage.createSubjectNoUID()`. UID dostane až v centrále.
...
- **Hromadný import stavov (Bulk Status Import Module)**: Multi-step Excel import for bulk updating contract statuses. Tables: `bulk_status_import_types` (configurable import type definitions with identifierType: proposalNumber/contractNumber/insuranceContractNumber), `bulk_status_import_sessions` (import sessions with status/stats), `bulk_status_import_rows` (per-row results). Page: `/hromadne-stavy` with tabs: Hromadný import (active sessions), Archív importov, Hromadné akcie. Multi-step wizard: Info → Upload Excel → Map columns → Execute → Results. Import types are configurable and appear dynamically in sidebar under "Hromadné stavy". On status change, `contract_status_change_logs` is updated. API endpoints: `/api/bulk-status-import-types` (CRUD), `/api/bulk-status-import-sessions` (CRUD + archive/unarchive), `/api/bulk-status-import/parse` (Excel parse), `/api/bulk-status-import-sessions/:id/execute` (execute import).
- **Holding Groups (Auto-sync Spoločnosti → Skupiny klientov)**: Every company in `my_companies` automatically gets a linked client group in `client_groups` with `isHoldingGroup=true`, `isSystem=true`, `linkedCompanyId=company.id`, `groupCode=holding_company_{id}`. Visual: red Lock icon + red "Holding" badge (vs amber for regular system groups). Member count = subjects where `myCompanyId = linkedCompanyId` (not `client_group_members`). DELETE blocked with 403. Name auto-synced on company update. Seed creates holding groups for all existing companies on startup.
