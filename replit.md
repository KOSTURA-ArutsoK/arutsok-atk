...
## Recent Features
- **Intelligent Dashboard (Inteligentný Dashboard)**: Modular analytics dashboard at `/`. Backend `GET /api/dashboard/analytics` endpoint with filters (dateFrom, dateTo, phase, inventoryId), state/company isolation. **Smart Filter Bar**: Fixed filter strip with time presets (Dnes, 7 dní, 30 dní, Kvartál, Rok, Všetko, Vlastný), lifecycle phase filter (F1-F10), sprievodka filter. Reactive — any filter change instantly re-renders all charts. **Quick Stats**: 3 cards (Celkový počet zmlúv, Nevybavené výhrady, Počet sprievodiek). **4 Analytics Charts** using recharts: Trend Skenovania (Area Chart, cumulative contract growth), Distribúcia Fáz (Donut Chart with strict colors: F5+=green, F3=red, F2=blue, F7=orange, other=gray), Kvalita Procesu (Stacked Bar Chart, accepted vs objections over time), Aktivita Protokolov (Bar Chart, sprievodky per month). All charts in Card bubbles with empty state placeholders. Existing drag-and-drop widgets preserved below charts.
