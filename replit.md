...
- **Status Notification Templates**: `contractStatuses` table has `notifyEnabled`, `notifyChannel` (email/sms/both), `notifySubject`, `notifyTemplate` columns. UI: "Notifikacie" tab in status edit dialog with switch, channel selector, subject field, smart tag buttons (`{{contract_number}}`, `{{client_name}}`, `{{valid_until}}`), and template editor. On status change, system auto-replaces smart tags with real data and inserts into `system_notifications` with `notificationType: "status_change_email"` or `"status_change_sms"`.
