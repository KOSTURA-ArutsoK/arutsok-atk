...
- **Module-Specific Features**:
    ...
    - **Status Change Modal (ArutsoK 51)**: 4-tab modal for contract status changes: Tab 1 (Vseobecne) - new status dropdown + datetime picker; Tab 2 (Povolenia) - visibleToClient toggle + statusNote textarea; Tab 3 (Parametre) - dynamic parameter inputs from `contract_status_parameters` with required validation; Tab 4 (Dokumenty) - multi-file uploader. All data stored in `contract_status_change_logs` with extended fields: `changedAt`, `visibleToClient`, `statusNote`, `statusChangeDocuments` (JSONB). `lastStatusUpdate` timestamp on `contracts`. Auto-assigns `globalNumber` if status has `assignsNumber=true`. Visual indicators (MessageSquare/Paperclip icons) in contract lists for entries with notes/documents. API: POST `/api/contracts/:id/status-change` (multipart with file upload), POST `/api/contracts/status-change-meta` (bulk meta for UI indicators). Component: `StatusChangeModal` at `client/src/components/status-change-modal.tsx`, integrated in ContractForm "stavy" tab via "Zmenit stav" button.
