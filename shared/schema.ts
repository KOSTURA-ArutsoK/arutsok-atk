...
export const myCompanies = pgTable("my_companies", {
  ...
  city: text("city"),
  corrStreet: text("corr_street"),
  corrStreetNumber: text("corr_street_number"),
  corrOrientNumber: text("corr_orient_number"),
  corrPostalCode: text("corr_postal_code"),
  corrCity: text("corr_city"),
  corrStateId: integer("corr_state_id"),
  branches: jsonb("branches").$type<{ name?: string; street?: string; streetNumber?: string; postalCode?: string; city?: string; stateId?: number; phone?: string; email?: string; phones?: string[]; emails?: string[]; isActive?: boolean; activeFrom?: string; cancelledAt?: string; branchStatus?: string; employees?: { photo?: string; uid?: string; position?: string; titleBefore?: string; firstName?: string; lastName?: string; titleAfter?: string; phone?: string; email?: string; status?: string; inactiveUntil?: string; statusHistory?: { status: string; from?: string; to?: string }[] }[] }[]>().default([]),
...
export const partners = pgTable("partners", {
  ...
  taxDocs: jsonb("tax_docs").$type<DocEntry[]>().default([]),
  businessActivities: jsonb("business_activities").$type<{ text: string; since?: string }[]>().default([]),
  branches: jsonb("branches").$type<any[]>().default([]),
  corrStreet: text("corr_street"),
  corrStreetNumber: text("corr_street_number"),
  corrOrientNumber: text("corr_orient_number"),
  corrPostalCode: text("corr_postal_code"),
...
export const subjects = pgTable("subjects", {
  ...
  registrationStatus: text("registration_status").$type<"potencialny" | "tiper" | "klient">().default("klient"),
...
export const products = pgTable("products", {
  ...
  description: text("description"),
  allowedSpecialists: integer("allowed_specialists").array(),
  allowedRecommenders: integer("allowed_recommenders").array(),
  ...
  requiredDocumentsReceived: jsonb("required_documents_received").$type<string[]>().default([]),
  optionalDocumentsReceived: jsonb("optional_documents_received").$type<string[]>().default([]),
  requiredDocumentsPartner: jsonb("required_documents_partner").$type<string[]>().default([]),
  optionalDocumentsPartner: jsonb("optional_documents_partner").$type<string[]>().default([]),
  isDeleted: boolean("is_deleted").default(false),
  deletedBy: text("deleted_by"),
  deletedAt: timestamp("deleted_at"),
  deletedFromIp: text("deleted_from_ip"),
...
export const contracts = pgTable("contracts", {
  ...
  signatoryTitleBefore: text("signatory_title_before"),
  signatoryFirstName: text("signatory_first_name"),
  signatoryLastName: text("signatory_last_name"),
  signatoryTitleAfter: text("signatory_title_after"),
...
export const insertContractSchema = createInsertSchema(contracts).omit({ id: true, createdAt: true, updatedAt: true, isDeleted: true, deletedBy: true, deletedAt: true, deletedFromIp: true, uid: true, isLocked: true, lockedBy: true, lockedAt: true, lockedBySupiskaId: true, subjectSnapshot: true, subjectSnapshotAt: true }).extend({ signedDate: z.coerce.date().nullable().optional(), effectiveDate: z.coerce.date().nullable().optional(), expiryDate: z.coerce.date().nullable().optional() });
...
export const subjectParameters = pgTable("subject_parameters", {
  ...
  rowId: integer("row_id").references(() => subjectParamSections.id),
  fieldKey: text("field_key").notNull(),
  code: text("code"),
...
export const contractParamVerifications = pgTable("contract_param_verifications", {
  ...
  contractId: integer("contract_id").notNull().references(() => contracts.id),
  paramKey: text("param_key").notNull(),
  status: text("status").$type<"pending" | "ok" | "not_ok" | "sync_subject" | "corrected_snapshot">().notNull().default("pending"),
  reason: text("reason").$type<"missing_mandatory_data" | "irresolvable_conflict">(),
